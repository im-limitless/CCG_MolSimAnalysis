...
# find index of proximal H atoms to O within 1.3 Angstroms
    ...
    if len(BondOH) == 3:
        ...
        HydroIdx = BondOH(find(np.amax(DistOH(BondOH))))
        ...
        print(np.array(['O atom ',num2str(AtomIndx.O(j)),' is bonded to 3 x H atoms ',num2str(BondOH(1)),' ',num2str(BondOH(2)),' ',num2str(BondOH(3))]))
        AtomsNew[AtomIndx.O[j],:] = pad('Li',AtomsNew.shape[2-1])
        AtomsNew[AtomIndx.H[BondOH[1]],:] = pad('S',AtomsNew.shape[2-1])
        AtomsNew[AtomIndx.H[BondOH[2]],:] = pad('S',AtomsNew.shape[2-1])
        AtomsNew[AtomIndx.H[BondOH[3]],:] = pad('S',AtomsNew.shape[2-1])
        #             HydroList = [HydroList; AtomIndx.H(HydroIdx) AtomIndx.O(j) AtomIndx.H(BondOH')];

fidout = open(np.array([BaseFldr,fldrname,'\New_',Trajectory]),'w')
fidout.write(np.array([num2str(nAtoms),newlinechar]) % ())
fidout.write(np.array([' i =   input geom',newlinechar]) % ())
for kk in np.arange(1,AtomsNew.shape[1-1]+1).reshape(-1):
    fidout.write(np.array([pad(AtomsNew(kk,:),8),pad(num2str(XYZ_snap(kk,1),'%.10g'),20),pad(num2str(XYZ_snap(kk,2),'%.10g'),20),pad(num2str(XYZ_snap(kk,3),'%.10g'),20),newlinechar]) % ())
