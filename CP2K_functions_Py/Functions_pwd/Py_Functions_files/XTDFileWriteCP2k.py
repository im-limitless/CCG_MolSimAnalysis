...
def XTDFileWriteCP2k(XTDFileName = None,n = None,AtomCur = None,AtomPosImages = None,Vec = None,MovieSpeed = None,Snapshots = None): 
...
# IDs NAtoms+9 -> reserved for ReciprocalLattice3D element
    ...
    tmpstr = sprintf('%1.0f,',np.array([np.arange(4,NAtoms + 3+1),NAtoms + 5]))
    SymmSys.setAttribute('Children',tmpstr(np.arange(1,end() - 1+1)))
    ...
    tmpstr = sprintf('%1.0f,',np.array([np.arange(4,NAtoms + 3+1)]))
    IdentMappng.setAttribute('MappedObjects',tmpstr(np.arange(1,end() - 1+1)))
    tmpstr = sprintf('%1.0f,',np.array([NAtoms + 5,NAtoms + 8]))
    IdentMappng.setAttribute('DefectObjects',tmpstr(np.arange(1,end() - 1+1)))
    ...
    for k in np.arange(1,len(n)+1).reshape(-1):
        for i in np.arange(1,n+1).reshape(-1):
            jAtom = sum(n(np.arange(1,k - 1+1))) + i
            ...
            NewAtom.setAttribute('Name',np.array([AtomCur[i],num2str(jAtom)]))
            ...
            NewAtom.setAttribute('XYZ',tmpstr(np.arange(1,end() - 1+1)))
    ...
    SpaceGrp.setAttribute('AVector',tmpstr(np.arange(1,end() - 1+1)))
    tmpstr = sprintf('%10.16f,',Vec(2,:))
    SpaceGrp.setAttribute('BVector',tmpstr(np.arange(1,end() - 1+1)))
    tmpstr = sprintf('%10.16f,',Vec(3,:))
    SpaceGrp.setAttribute('CVector',tmpstr(np.arange(1,end() - 1+1)))
    ...
    Trajc.setAttribute('End',num2str(AtomPosImages.shape[3-1]))
    Trajc.setAttribute('Type','arc')
    if MovieSpeed < 0:
        warnings.warn('Out of range MovieSpeed value fixed to 0.')
        MovieSpeed = 0
    
    if MovieSpeed > 10:
        warnings.warn('Out of range MovieSpeed value fixed to 10.')
        MovieSpeed = 10
    ...
    newlinechar = np.array([char(13),char(10)])
    fidout = open(XTDFileName.replace('.xtd','.arc'),'w')
    fwrite(fidout,np.array(['!BIOSYM archive 3',newlinechar]))
    fwrite(fidout,np.array(['PBC=ON',newlinechar]))
    # for k = 1:size(AtomPosImages,3)
    for k in np.arange(1,len(Snapshots)+1).reshape(-1):
        vecls = np.array([norm(Vec(1,:)),norm(Vec(2,:)),norm(Vec(3,:))])
        angls = np.array([angle(Vec(2,:),Vec(3,:)),angle(Vec(1,:),Vec(3,:)),angle(Vec(1,:),Vec(2,:))])
        fwrite(fidout,np.array([np.matlib.repmat(' ',1,74),'0.0000',newlinechar]))
        fwrite(fidout,np.array(['!DATE     Dec 16 10:36:30 2011',newlinechar]))
        fwrite(fidout,'PBC')
        for num1 in np.array([vecls,angls]).reshape(-1):
            fwrite(fidout,np.array([padwithspaces(num2str(num1,'%15.4f'),10,'right')]))
        fwrite(fidout,newlinechar)
        PBCbs[1,:] = np.array([vecls(1),0,0])
        PBCbs[2,:] = np.array([np.cos(pi / 180 * angls(3)),np.sin(pi / 180 * angls(3)),0]) * vecls(2)
        PBCbs[3,1] = vecls(3) * np.cos(pi / 180 * angls(2))
        PBCbs[3,2] = (vecls(2) * vecls(3) * np.cos(pi / 180 * angls(1)) - PBCbs(2,1) * PBCbs(3,1)) / PBCbs(2,2)
        PBCbs[3,3] = np.sqrt(vecls(3) ** 2 - PBCbs(3,1) ** 2 - PBCbs(3,2) ** 2)
        for i in np.arange(1,n+1).reshape(-1):
            #         for j = 1:n(i)
...
#             posv(3) = AtomPosImages(i,3,k);
            ...
            #             posw = posv*PBCbs;
            strline = np.array([padwithspaces(AtomCur[i],5,'left'),padwithspaces(num2str(posv(1),'% 15.9f'),15,'right'),padwithspaces(num2str(posv(2),'% 15.9f'),15,'right'),padwithspaces(num2str(posv(3),'% 15.9f'),15,'right'),' XXXX 1      xx      ',padwithspaces(AtomCur[i],2,'left'),'  0.000'])
            fwrite(fidout,np.array([strline,newlinechar]))
            #         end
        fwrite(fidout,np.array(['end',newlinechar]))
        fwrite(fidout,np.array(['end',newlinechar]))
...
def CPK_or_BnS(in1 = None): 
    if strcmpi(in1,'Cu') or strcmpi(in1,'Al') or strcmpi(in1,'Alb') or strcmpi(in1,'Al1') or strcmpi(in1,'Al2') or strcmpi(in1,'Ag') or strcmpi(in1,'Au') or strcmpi(in1,'Pd') or strcmpi(in1,'Pts') or strcmpi(in1,'Ptss') or strcmpi(in1,'Ptb') or strcmpi(in1,'PtE') or strcmpi(in1,'Pt') or contains(in1,'Ir') or strcmpi(in1,'Li') or strcmpi(in1,'S') or contains(in1,'Ru'):
        out1 = 'CPK'
    else:
        if strcmpi(in1,'C') or strcmpi(in1,'H') or strcmpi(in1,'B') or strcmpi(in1,'O') or contains(in1,'O') or strcmpi(in1,'Mo') or strcmpi(in1,'N'):
            out1 = 'Stick'
        else:
            if strcmpi(in1,'Cl') or strcmpi(in1,'F') or strcmpi(in1,'Na'):
                out1 = 'CPK'
    
    return out1
    
    
def angle(a = None,b = None): 
    out1 = 180 / pi * atan2(norm(cross(a,b)),np.dot(a,b))
    return out1
    
    
def padwithspaces(stringin = None,totallength = None,alignment = None): 
    spcs = np.matlib.repmat(' ',1,np.amax(0,totallength - len(stringin)))
    if strcmpi(alignment,'right'):
        out1 = np.array([spcs,stringin])
    else:
        if strcmpi(alignment,'left'):
            out1 = np.array([stringin,spcs])
        else:
