...
def CreateCP2KInputfile(Base = None,OutFldr = None,Vector = None,Wall = None,FixList = None,RestrainList = None,Calc = None,Velocities = None,Elems = None,WaveFunc = None): 
    # write submission script
    
    fidout = open(np.array([Base,OutFldr,'\',OutFldr,'-1.restart']),'w')
    ...
    if np.logical_or(str(Calc) == str('BOMD'),str(Calc) == str('CP')):
        ...
        if str(Calc) == str('BOMD'):
            ...
            fidout.write(np.array(['       TYPE CSVR',newline]) % ())
    ...
    if not len(FixList)==0  or not len(RestrainList)==0 :
        fidout.write(np.array(['   &CONSTRAINT',newline]) % ())
        if not len(FixList)==0 :
        ...
        if not len(RestrainList)==0 :
            fidout.write(np.array(['     &FIXED_ATOMS',newline]) % ())
            fidout.write(np.array(['       LIST  ',num2str(np.transpose(RestrainList)),newline]) % ())
            fidout.write(np.array(['       COMPONENTS_TO_FIX XYZ',newline]) % ())
            fidout.write(np.array(['       &RESTRAINT',newline]) % ())
            fidout.write(np.array(['         K     2.5727147833353803E-04',newline]) % ())
            fidout.write(np.array(['       &END RESTRAINT',newline]) % ())
            fidout.write(np.array(['     &END FIXED_ATOMS',newline]) % ())
...
#     fprintf(fidout,['   &END EXTERNAL_POTENTIAL' newline]);
    ...
    if str(Calc) == str('BOMD'):
        ...
        if str(Calc) == str('CP'):
            ...
            if str(Calc) == str('GEO'):
                fidout.write(np.array(['       SCF_GUESS  RESTART',newline]) % ())
                fidout.write(np.array(['       MAX_SCF  20',newline]) % ())
                fidout.write(np.array(['       &OUTER_SCF',newline]) % ())
                fidout.write(np.array(['         MAX_SCF 400',newline]) % ())
                fidout.write(np.array(['         EPS_SCF     3.0E-07',newline]) % ())
                fidout.write(np.array(['       &END OUTER_SCF',newline]) % ())
                fidout.write(np.array(['       EPS_SCF     3.0E-07',newline]) % ())
    ...
    fidout.write(np.array(['       &XC_FUNCTIONAL',newline]) % ())
    fidout.write(np.array(['         &PBE',newline]) % ())
    fidout.write(np.array(['           PARAMETRIZATION ORIG',newline]) % ())
    ...
    if np.logical_and(not len(Velocities)==0 ,(np.logical_or(str(Calc) == str('BOMD'),str(Calc) == str('CP')))):
    ...
    if str(WaveFunc) == str('Yes'):
        ...
        if str(WaveFunc) == str('No'):
            copyfile(np.array([Base,OutFldr,'\',OutFldr,'-1.restart']),np.array([Base,OutFldr,'\',OutFldr,'.inp']))
