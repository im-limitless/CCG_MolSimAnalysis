function CreateCP2KInputfile(Base, OutFldr, Vector, Wall, FixList, RestrainList, Calc, Velocities, Elems, WaveFunc)

% write submission script

fidout = fopen([Base OutFldr '\' OutFldr '-1.restart'],'w');
...
if strcmp(Calc, 'BOMD') | strcmp(Calc, 'CP')
    ...
    if strcmp(Calc, 'BOMD')
        ...
        fprintf(fidout,['       TYPE CSVR' newline]);
...
if ~isempty(FixList) || ~isempty(RestrainList)
    ...
    if ~isempty(RestrainList)
        fprintf(fidout,['     &FIXED_ATOMS' newline]);
        fprintf(fidout,['       LIST  ' num2str(RestrainList') newline]);
        fprintf(fidout,['       COMPONENTS_TO_FIX XYZ' newline]);
        fprintf(fidout,['       &RESTRAINT' newline]);
        fprintf(fidout,['         K     2.5727147833353803E-04' newline]);
        fprintf(fidout,['       &END RESTRAINT' newline]);
        fprintf(fidout,['     &END FIXED_ATOMS' newline]);
    end
...
elseif strcmp(Calc, 'GEO')
    fprintf(fidout,['       SCF_GUESS  RESTART' newline]);
    fprintf(fidout,['       MAX_SCF  20' newline]);
    fprintf(fidout,['       &OUTER_SCF' newline]);
    fprintf(fidout,['         MAX_SCF 400' newline]);
    fprintf(fidout,['         EPS_SCF     3.0E-07' newline]);
    fprintf(fidout,['       &END OUTER_SCF' newline]);
    fprintf(fidout,['       EPS_SCF     3.0E-07' newline]);
end
...
fprintf(fidout,['       &XC_FUNCTIONAL' newline]);
fprintf(fidout,['         &PBE' newline]);
fprintf(fidout,['           PARAMETRIZATION ORIG' newline]);
...
if ~isempty(Velocities) & (strcmp(Calc, 'BOMD') | strcmp(Calc, 'CP'))
...
if strcmp(WaveFunc, 'Yes')
    ...
    fclose(fidout);
elseif strcmp(WaveFunc, 'No')
    copyfile([Base OutFldr '\' OutFldr '-1.restart'], [Base OutFldr '\' OutFldr '.inp'])
end