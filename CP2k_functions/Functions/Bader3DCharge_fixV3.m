function Bader3DCharge_fixV3(TrajXYZ, TrajABC, TrajQmc, varargin)
...
[hAtoms, hVectors] = updatePlot(ax, TrajXYZ{1}, TrajABC{1}, TrajQmc{1},...
    xs, ys, zs, Radius, SelectedIndices, currentFrame, numFrames);

% Video export routine
if params.SaveVideo
    for frame = 1:numFrames
        [hAtoms, hVectors] = updatePlot(ax, TrajXYZ{frame}, TrajABC{frame},...
            TrajQmc{frame}, xs, ys, zs, Radius, SelectedIndices, frame, numFrames);
    ...
    function autoPlayAnimation()
        for frame = 1:numFrames
            [hAtoms, hVectors] = updatePlot(ax, TrajXYZ{frame},...
                ...
                xs, ys, zs, Radius, SelectedIndices, frame, numFrames);
            if params.Interactive
                set(findobj(fig,'Tag','frameSlider'), 'Value', frame);
            end
            pause(1/params.FrameRate);
...
function [hAtoms, hVectors] = updatePlot(ax, XYZ, ABC, Qmc, xs, ys, zs, Radius, SelectedIndices, currentFrame, numFrames)
% Update plot elements with proper handle management
delete(findobj(ax,'Type','Surface'));
delete(findobj(ax,'Type','Line','-not','-regexp','Tag',''));
...
% Plot complete cell vectors
Vec = diag(ABC);
LwD = 2;
hVectors = gobjects(0); % Initialize empty graphics array

% Base vectors
hVectors(end+1) = plot3(ax, [0 Vec(1,1)],[0 Vec(1,2)],[0 Vec(1,3)],'-k','LineWidth',LwD);
hVectors(end+1) = plot3(ax, [0 Vec(2,1)],[0 Vec(2,2)],[0 Vec(2,3)],'-k','LineWidth',LwD);
hVectors(end+1) = plot3(ax, [0 Vec(3,1)],[0 Vec(3,2)],[0 Vec(3,3)],'-k','LineWidth',LwD);

% Additional edges
hVectors(end+1) = plot3(ax, Vec(1,1)+[0 Vec(3,1)],Vec(1,2)+[0 Vec(3,2)],Vec(1,3)+[0 Vec(3,3)],'-k','LineWidth',LwD);
hVectors(end+1) = plot3(ax, Vec(2,1)+[0 Vec(3,1)],Vec(2,2)+[0 Vec(3,2)],Vec(2,3)+[0 Vec(3,3)],'-k','LineWidth',LwD);
hVectors(end+1) = plot3(ax, Vec(1,1)+Vec(2,1)+[0 Vec(3,1)],Vec(1,2)+Vec(2,2)+[0 Vec(3,2)],Vec(1,3)+Vec(2,3)+[0 Vec(3,3)],'-k','LineWidth',LwD);
hVectors(end+1) = plot3(ax, Vec(3,1)+[0 Vec(1,1)],Vec(3,2)+[0 Vec(1,2)],Vec(3,3)+[0 Vec(1,3)],'-k','LineWidth',LwD);
hVectors(end+1) = plot3(ax, Vec(3,1)+[0 Vec(2,1)],Vec(3,2)+[0 Vec(2,2)],Vec(3,3)+[0 Vec(2,3)],'-k','LineWidth',LwD);
hVectors(end+1) = plot3(ax, Vec(3,1)+Vec(1,1)+[0 Vec(2,1)],Vec(3,2)+Vec(1,2)+[0 Vec(2,2)],Vec(3,3)+Vec(1,3)+[0 Vec(2,3)],'-k','LineWidth',LwD);
hVectors(end+1) = plot3(ax, Vec(3,1)+Vec(2,1)+[0 Vec(1,1)],Vec(3,2)+Vec(2,2)+[0 Vec(1,2)],Vec(3,3)+Vec(2,3)+[0 Vec(1,3)],'-k','LineWidth',LwD);
