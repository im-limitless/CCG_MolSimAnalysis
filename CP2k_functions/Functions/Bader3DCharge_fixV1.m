function Bader3DCharge_fixV1(XYZ, ABC, Qmc) 
%NOTE: this version the coloring and color bar are fixed such that the red and
%blue gradients are only for negative and positive values respectively.
...
% Handle case where all charges are the same
if max_Q == min_Q
    if max_Q > 0
        cmap = [0 0 1]; % Blue for all positive
    elseif max_Q < 0
        cmap = [1 0 0]; % Red for all negative
    else
        cmap = [0 0.8 0]; % Green for all zero
    end
    cmap_length = 1;
else
    has_neg = any(Qmc < 0);
    ...
    elseif has_pos
        x_points = [0, max_Q];
        color_points = [0 0.8 0; 0 0 1]; % Green to Blue
    else
        x_points = [min_Q, 0];
        color_points = [1 0 0; 0 0.8 0]; % Red to Green
    end
...
for i = 1:size(XYZ, 1)
    ...
    hsurf = surf(xAt, yAt, zAt, 'FaceColor', cmap(C,:), 'EdgeColor','None');
...
% Plotting cell vectors and other annotations remain unchanged
Vec = diag(ABC);
LwD = 2;
...
% Additional cell vector plotting code here...
