function Bader3DCharge_fixV3_8_5(TrajXYZ, TrajABC, TrajQmc, varargin)
...
% Process SelectedIndices into a cell array of vectors per frame
if isempty(SelectedIndices)
    % Default to all atoms for all frames
    SelectedIndices = repmat({(1:Natoms)'}, numFrames, 1);
elseif iscell(SelectedIndices)
    % Validate each cell is a vector and number of cells matches numFrames
    if numel(SelectedIndices) ~= numFrames
        error('SelectedIndices cell array must have one cell per frame.');
    end
    % Ensure each cell is a column vector
    for i = 1:numFrames
        if ~isvector(SelectedIndices{i})
            error('SelectedIndices{%d} is not a vector.', i);
        end
        SelectedIndices{i} = SelectedIndices{i}(:); % Convert to column vector
    end
else
    % SelectedIndices is a vector; repeat for all frames
    SelectedIndices = repmat({SelectedIndices(:)}, numFrames, 1);
end
...
[hAtoms, hVectors] = updatePlot(ax, TrajXYZ{1}, TrajABC{1}, TrajQmc{1},...
    xs, ys, zs, Radius, SelectedIndices{1}, currentFrame, numFrames, hAtoms, hVectors);

% Video export
if params.SaveVideo
    for frame = 1:numFrames
        [hAtoms, hVectors] = updatePlot(ax, TrajXYZ{frame}, TrajABC{frame},...
            TrajQmc{frame}, xs, ys, zs, Radius, SelectedIndices{frame}, frame, numFrames, hAtoms, hVectors);
    ...
    function playCallback(src)
        ...
        while loopActive && ishandle(src)
            ...
            [hAtoms, hVectors] = updatePlot(ax, TrajXYZ{currentFrame},...
                ...
                xs, ys, zs, Radius, SelectedIndices{currentFrame}, currentFrame, numFrames, hAtoms, hVectors);
        ...
        while params.PlayVideo && (params.LoopVideo || playCount == 0)
            for frame = 1:numFrames
                ...
                [hAtoms, hVectors] = updatePlot(ax, TrajXYZ{frame},...
                    ...
                    xs, ys, zs, Radius, SelectedIndices{frame}, frame, numFrames, hAtoms, hVectors);
