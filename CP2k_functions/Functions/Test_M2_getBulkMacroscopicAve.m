...
for k=[3:1.5:10]
    ...
    z_median= find(z>=(limits(1)-2) & z<=(limits(2)-2));
...
%             Macro_AveDens_T=[k,j,median(AveTotDen(z_indx))];
            Macro_AveDens_T=[k,j,median(AveTotDen(z_median))];
            Temp_den= smooth(Temp_den,j);
            AveTotDen(z_indx)= Temp_den(z_indx);
            plot(z,AveTotDen,  'linewidth', 1.5)
            if i==iterations & j==20
                text(z(end)/2, 1500, ['Bulk Density (last)= ' num2str(median(AveTotDen(z_median)),'%.0f') ' kgm^{-3}'],'HorizontalAlignment','center','FontSize',14)
            end
...
writetable(T, [BaseFldr Allfldrs.name PathSep system '_Macro_M2_V2.xlsx'])
