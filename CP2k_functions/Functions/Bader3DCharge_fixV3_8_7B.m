function Bader3DCharge_fixV3_8_7B(TrajXYZ, TrajABC, TrajQmc, varargin)
...
% Input parsing (unchanged until ExcludedIndices processing)
% ... [previous input parsing code] ...

% ==== CHANGE 1: Keep excluded indices separate instead of subtracting ====
% Remove the exclusion subtraction from CombinedIndices computation
% Original line: frameIndices = setdiff(frameIndices, excluded);
% New code keeps exclusions separate
...
% Keep excluded indices separate
ExcludedIndices = processedExcluded;  % This is already cell array per frame
...
xlabel(ax, 'X-axis', 'FontSize', fontSize);
ylabel(ax, 'Y-axis', 'FontSize', fontSize);
zlabel(ax, 'Z-axis', 'FontSize', fontSize);

% Add colorbar with font size
hColorbar = colorbar(ax);
hColorbar.FontSize = fontSize;
% =======================================
...
% ==== CHANGE 2: Modified updatePlot calls to include ExcludedIndices ====
% Initial plot call
[hAtoms, hVectors] = updatePlot(ax, TrajXYZ{1}, TrajABC{1}, TrajQmc{1},...
    xs, ys, zs, Radius, CombinedIndices{1}, ExcludedIndices{1},...  % Added ExcludedIndices
    currentFrame, numFrames, hAtoms, hVectors, fontSize);

% Video export loop
if params.SaveVideo
    for frame = 1:numFrames
        [hAtoms, hVectors] = updatePlot(ax, TrajXYZ{frame}, TrajABC{frame},...
            TrajQmc{frame}, xs, ys, zs, Radius, CombinedIndices{frame}, ExcludedIndices{frame},...  % Added
            frame, numFrames, hAtoms, hVectors, fontSize);
        % ... [rest unchanged] ...
...
% ... [animation control callbacks unchanged except for updatePlot calls] ...
    ...
    function playCallback(src)
        ...
        while loopActive && ishandle(src)
            ...
            [hAtoms, hVectors] = updatePlot(ax, TrajXYZ{currentFrame},...
                ...
                xs, ys, zs, Radius, CombinedIndices{currentFrame}, ExcludedIndices{currentFrame}, currentFrame, numFrames, hAtoms, hVectors, fontSize);
        ...
        while params.PlayVideo && (params.LoopVideo || playCount == 0)
            for frame = 1:numFrames
                ...
                [hAtoms, hVectors] = updatePlot(ax, TrajXYZ{frame},...
                    ...
                    xs, ys, zs, Radius, CombinedIndices{frame}, ExcludedIndices{currentFrame}, frame, numFrames, hAtoms, hVectors, fontSize);
...
%% ==== CHANGE 3: Updated updatePlot function =============================
function [hAtoms, hVectors] = updatePlot(ax, XYZ, ABC, Qmc, xs, ys, zs, Radius,...
    CombinedIndices, ExcludedIndices, currentFrame, numFrames, hAtoms, hVectors, fontSize)  % Added ExcludedIndices

delete(findobj(ax,'Type','text','Tag','FrameText'));

% ==== CHANGE 4: Colormap based on selected non-excluded atoms ====
validSelected = setdiff(CombinedIndices, ExcludedIndices);
if ~isempty(validSelected)
    [cmap, minQ, maxQ] = createColormap(Qmc(validSelected));
else
    cmap = [0.7 0.7 0.7];  % Gray default
    minQ = 0;
    maxQ = 1;
end
colormap(ax, cmap);
...
% ==== CHANGE 5: Update atom creation/update with new logic ====
    if isempty(hAtoms) || ~isvalid(hAtoms(1))
        hAtoms = createAtoms(ax, XYZ, xs, ys, zs, Radius, Qmc,...
            CombinedIndices, ExcludedIndices, cmap, minQ, maxQ);  % Added ExcludedIndices
    else
        updateAtoms(hAtoms, XYZ, xs, ys, zs, Radius, Qmc,...
            CombinedIndices, ExcludedIndices, cmap, minQ, maxQ);  % Added ExcludedIndices
    end
end
...
%% ==== CHANGE 6: Modified atom creation function =========================
function hAtoms = createAtoms(ax, XYZ, xs, ys, zs, Radius, Qmc,...
    CombinedIndices, ExcludedIndices, cmap, minQ, maxQ)  % Added ExcludedIndices
...
for i = 1:size(XYZ,1)
    % ==== NEW LOGIC =====================================================
    if ismember(i, ExcludedIndices)
        % Case 1: Excluded atom - hide completely
        visible = 'off';
        color = [0 0 0];  % Arbitrary color (not shown)
        alpha = 1;
    elseif ismember(i, CombinedIndices)
        % Case 2: Selected atom - color normally
        if maxQ ~= minQ
        ...
        % Case 3: Unselected atom - gray out
        color = [0.7 0.7 0.7];  % Gray color
...
%% ==== CHANGE 7: Modified atom update function ===========================
function updateAtoms(hAtoms, XYZ, xs, ys, zs, Radius, Qmc,...
    CombinedIndices, ExcludedIndices, cmap, minQ, maxQ)  % Added ExcludedIndices

for i = 1:size(XYZ,1)
    % ==== NEW LOGIC =====================================================
    if ismember(i, ExcludedIndices)
        % Case 1: Excluded atom
        visible = 'off';
        color = [0 0 0];
        alpha = 1;
    elseif ismember(i, CombinedIndices)
        % Case 2: Selected atom
        if maxQ ~= minQ
            cidx = round((Qmc(i) - minQ)/(maxQ - minQ) * (size(cmap,1)-1)) + 1;
        else
            cidx = 1;
