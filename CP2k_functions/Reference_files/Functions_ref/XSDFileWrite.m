function XSDFileWrite(XSDFileName,n,AtomCur,AtomPos,Vec)
...
for k = 1:length(n)
    for i = 1:n(k)
        ...
        NewAtom.setAttribute('Name',[AtomCur{k} num2str(jAtom)]);
        NewAtom.setAttribute('UserID',num2str(jAtom));
        NewAtom.setAttribute('DisplayStyle',CPK_or_BnS(AtomCur{k}));
        tmpstr = sprintf('%10.16f,',AtomPos(k,i,:));
        NewAtom.setAttribute('XYZ',tmpstr(1:end-1));
        NewAtom.setAttribute('Components',AtomCur{k});
