...
for i = 1:length(StepNum)
    if any(StepNum(i) == StepNum_Traj)
        ...
        if END1 == 'y'
            Oxide_sum(i) = sum(Qnet(Oxide{Inter},i));
            Oxide_mean(i) = mean(Qnet(Oxide{Inter},i));
            Oxide_std(i) = std(Qnet(Oxide{Inter},i));
        end
...
if END1 == 'y'
    legend('1st Water Layer', '<1st Water Layer>', '2nd Water Layer', '<2nd Water Layer>', 'Bulk Water', '<Bulk Water>', 'Oxide', '<Oxide>', AtomList(AlList(1),:), AtomList(AlList(2),:), AtomList(AlList(3),:), 'interpreter', 'tex')
else
...
plot(StepNum/2000, totalAlsCharge/total_Al, '-o', 'color', 'k', 'markeredgecolor', 'k', 'markerfacecolor', 'c');
plot([StepNum(1)/2000 StepNum(end)/2000], [mean(totalAlsCharge/total_Al) mean(totalAlsCharge/total_Al)], '--', 'color', 'c');
ylabel('Excess Charge (|e|) per Water/Al (Note: Oxide is per atom');
...
if END1 == 'y'
    ...
    legend('1st Water Layer', '<1st Water Layer>', '2nd Water Layer', '<2nd Water Layer>', 'Bulk Water', '<Bulk Water>', 'Al Layers', '<Al Layers>', 'Oxide Layer', '<Oxide Layer>', 'interpreter', 'tex')
