...
for i = 1:nConfigs
    ...
    H2O_Coverage(i) = sum(GR(GR == 2))/2; %divided by 2 cause this counts the H atoms where each water has 2H
...
disp(['Ave. coverage of OH = ' num2str(mean(OH_Coverage(36:end)/108), '%.2f') ' +/- ' num2str(std(OH_Coverage(36:end)/108), '%.2f') ' ML'])
disp(['Ave. coverage of H2O = ' num2str(mean(H2O_Coverage(36:end)/108), '%.2f') ' +/- ' num2str(std(H2O_Coverage(36:end)/108), '%.2f') ' ML'])
disp(['Ave. coverage of H3O = ' num2str(mean(H3O_Coverage(36:end)/108), '%.2f') ' +/- ' num2str(std(H3O_Coverage(36:end)/108), '%.2f') ' ML'])
disp(['Ave. Total coverage = ' num2str(mean(Total_Coverage(36:end)/108), '%.2f') ' +/- ' num2str(std(Total_Coverage(36:end)/108), '%.2f') ' ML'])
disp(['Ave. O coverage = ' num2str(mean(O_Coverage(36:end)/108), '%.2f') ' +/- ' num2str(std(O_Coverage(36:end)/108), '%.2f') ' ML'])